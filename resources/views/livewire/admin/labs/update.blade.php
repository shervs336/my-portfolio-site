<div class="card">
    ...
    <form class="w-full max-w-lg">
            ...
            <div class="form-group mb-3">
                <label class="block tracking-wide uppercase text-gray-700 font-bold text-xs mb-2" for="excerpt">Link:</label>
                <textarea class="apperance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white  @error('link') is-invalid @enderror" id="link" wire:model="link" placeholder="Enter link"></textarea>
                @error('link')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            <div class="d-grid gap-2">
                <button wire:click.prevent="updateLab()" class="shadow uppercase text-xs tracking-wide text-white bg-purple-500 hover:bg-purple-400 focus:shadow-outline focus:outline-none font-bold py-2 px-4 rounded">Update</button>
                <button wire:click.prevent="cancelLab()" class="shadow uppercase text-xs tracking-wide text-white bg-red-500 hover:bg-red-400 focus:shadow-outline focus:outline-none font-bold py-2 px-4 rounded">Cancel</button>
            </div>
        </form>
    </div>
</div>